const _Node = require('./queue2-node');
...
class QueueTwo {
    ...
    enqueue(data) {
        ...
        if (this.last) {
            // the previously-last item now points its next to new node,
            this.last.next = node;

            // and the new, now-last item must point its `previous` to the formerly-last ndoe:
    ...
    dequeue() {
        ...
        // if the queue still has 1+ nodes, 
        //    what is now the "first" node in the queue has no "previous" node
        if (this.first) {
